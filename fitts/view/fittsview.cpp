...
void FittsView::createMenus() {
    ...
    actionShortcuts = new QAction(tr("Raccourcis clavier"), this);
...
void FittsView::createTooltips() {
    if (aValue)
        aValue->setToolTip("Paramètre a : influence de la distance dans la loi de Fitts");

    if (bValue)
        bValue->setToolTip("Paramètre b : constante représentant le temps minimal");

    if (nbCible)
        nbCible->setToolTip("Nombre total de cibles à atteindre pendant le test");

    if (minSize)
        minSize->setToolTip("Taille minimale des cibles");

    if (maxSize)
        maxSize->setToolTip("Taille maximale des cibles");
}
...
void FittsView::initWindows() {
    ...
    buttonStartFitts = new QToolButton;
    ...
    mainScreenAppChooserLayout->addWidget(buttonStartFitts);
